...
router.post("/data/purchase", authenticate, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (!isKybdataConfigured()) { res.status(503).json({ error: "KYB Data provider is not configured. Please contact admin." }); return; }
  ...
  if (!delivered) {
    ...
    await db.insert(transactionsTable).values({
      userId: req.userId!, type: "data", status: "failed",
  ...
  const [tx] = await db.insert(transactionsTable).values({
    userId: req.userId!, type: "data", status: "success",
    amount: price.toString(),
...
router.post("/electricity/purchase", authenticate, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (!delivered) {
    await db.update(walletsTable).set({ balance: sql`balance + ${amount}`, updatedAt: new Date() }).where(eq(walletsTable.userId, req.userId!));
    await db.insert(transactionsTable).values({
      userId: req.userId!, type: "electricity", status: "failed",
      amount: amount.toString(),
  ...
  const [tx] = await db.insert(transactionsTable).values({
    userId: req.userId!, type: "electricity", status: "success",
    amount: amount.toString(),
...
// plan_id values from KYB Data GET /api/v2/services/cable — use eaPlanId as reference until synced
const CABLE_PLANS = [
...
router.post("/exam/purchase", authenticate, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (!supportedBoards.some((b) => code.includes(b))) {
    res.status(503).json({ error: `${examType.name} tokens are not yet available. Please contact support on 09026329296.` });
    return;
  }
  ...
  if (!delivered) {
    await db.update(walletsTable).set({ balance: sql`balance + ${totalCost}`, updatedAt: new Date() }).where(eq(walletsTable.userId, req.userId!));
    await db.insert(transactionsTable).values({
      userId: req.userId!, type: "exam", status: "failed",
  ...
  const [tx] = await db.insert(transactionsTable).values({
    userId: req.userId!, type: "exam", status: "success",
    amount: totalCost.toString(),
  ...
  res.json({
    id: tx.id, status: "success", pins, examType: examType.code,
    amount: totalCost, createdAt: tx.createdAt,
  });
});
